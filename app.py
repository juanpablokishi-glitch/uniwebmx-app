...
carrusel_orientacion_url = f"{_SUPABASE_STORAGE_BASE}fondo_carrusel_orientacion.png"
...
<style>
   ...
   * {{
   ...
   /* Ya NO ocultamos header[data-testid="stHeader"] por completo con
      display:none. Ahí es donde vive el botón nativo de Streamlit para
      REABRIR la sidebar cuando está colapsada (pasa automáticamente en
      pantallas angostas, sin importar initial_sidebar_state="expanded").
      Con display:none ese botón desaparecía del DOM y en celular, donde la
      sidebar arranca colapsada, no había forma de volver a abrirla tras
      iniciar sesión. Ahora lo dejamos sin ocupar espacio visual pero
   ...
   /* Botón nativo de Streamlit para abrir/cerrar la sidebar: nos aseguramos
      de que siempre sea visible, clickeable y quede por encima de todo lo
      demás (algunos de nuestros z-index del sidebar-bottom-bar y del menú
      de perfil andan altos, así que este va más alto todavía). */
   ...
   /* Al quitar el header, recuperamos el espacio que dejaba arriba.
      ...
      espacio con un margen negativo agresivo, porque el header ya no se
      oculta con display:none — ahora solo queda transparente y con una
      altura fija chica. */
   .stApp {{
   ...
   @media (max-width: 768px) {{
       ...
       /* Asegura que el botón nativo de abrir/cerrar sidebar quede en una
          esquina accesible y no se tape con el contenido en pantallas
          angostas, donde es la ÚNICA forma de volver a abrir la sidebar. */
       [data-testid="collapsedControl"] {{
           top: 6px !important;
           left: 6px !important;
       }}
...
if es_hub:
   ...
   with st.sidebar:
       ...
       _html_items_analisis = _sb_item("Orientación Vocacional", "orientacion", _icon_orient) + _sb_item("Simulador", "simulador", _icon_sim)
...
if st.session_state.page == "inicio":
   ...
   _carrusel_slides = [
       ...
       ("Orientación vocacional", "[PENDIENTE: confirmar texto de esta tarjeta]", carrusel_orientacion_url, "linear-gradient(135deg,#4A5D32,#7C8A6A)"),
   ]
